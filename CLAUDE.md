...
- `win-monitor/win-monitor-tray.ps1` is what actually runs in the background day to day: it launches `win-monitor.ps1` as a separate hidden process and shows a system tray icon (drawn at runtime via `System.Drawing`, no `.ico` asset) with Open log folder / Open log viewer / Exit. Exit writes the stop-flag file rather than killing the child process, so the current session gets flushed. `win-monitor.ps1` itself is unchanged by this and still runs standalone via `win-monitor.cmd` or a console for foreground testing.
