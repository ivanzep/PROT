...
- `affinity-scripts/` — JavaScript scripts for Affinity Photo/Designer/Publisher's built-in scripting engine (run from Affinity's Scripts panel, not a browser), one subfolder per script. `affinity-scripts/Image Relink/README.md` covers install steps and per-script notes.
